...
public:
	...
	virtual uint8_t GetResponse(uint8_t* outPayload) {}
...
public:
	...
	virtual bool OnReceived(const uint8_t subHeader, const uint32_t timestamp, uint8_t* payload)
	{
		...
		case LOLA_LINK_CLOCKSYNC_ESTIMATION_REQUEST:
			...
			EstimationError = ATUI.uint - ((SyncedClock->GetSyncMicros() - (micros() - timestamp)) % UINT32_MAX);
	...
	// Returns sub-header.
	uint8_t GetResponse(uint8_t* outPayload)
	{
		...
		case LOLA_LINK_CLOCKSYNC_FULL_CLOCK_RESPONSE:
			...
			ATUI.uint = SyncedClock->GetSyncMicros() % UINT32_MAX;
